...
struct LoggedInView: View {
    ...
    func deleteAktivitaet(id: Int,art:String) {
        guard let url = URL(string: "http://localhost:8080/api/v1/aktivitaet/\(id)") else {
            print("Ungültige URL")
            return
        }
    ...
    func getAktivitaeten(name:String) {
        guard let url = URL(string: "http://localhost:8080/api/v1/aktivitaet/withArt/\(email)/\(name)") else {
            print("Invalid URL")
            return
        }
